import { db, watchlistTable, tradeLogsTable, lessonsTable } from "@workspace/db";
...
async function reconcilePaperPositions(figi: string, currentPrice: number): Promise<void> {
  ...
  for (const pos of open) {
    ...
    if (closeReason) {
      ...
      void journalClosedTrade({
        ticker: pos.ticker,
        figi: pos.figi,
        action: pos.action as "buy" | "sell",
        entry,
        exit: currentPrice,
        qty,
        pnl,
        closeReason,
        relatedTradeLogId: pos.id,
        reasoningAtEntry: pos.aiReasoning,
      });
    }
  }
}
...
/**
 * Lessons journal: short, AI-written one-liners summarising why a closed trade
 * worked or failed. Generated on close (only for "notable" outcomes — stop-loss
 * or |pnl| > 1% of notional) and injected into future per-ticker prompts to
 * counter recurring mistakes.
 */
async function journalClosedTrade(args: {
  ticker: string;
  figi: string;
  action: "buy" | "sell";
  entry: number;
  ...
  pnl: number;
  closeReason: string;
  relatedTradeLogId: number;
  reasoningAtEntry?: string | null;
}): Promise<void> {
  try {
    const notional = args.entry * args.qty;
    const pnlPct = notional > 0 ? (args.pnl / notional) * 100 : 0;
    const isNotable = args.closeReason === "stop_loss" || Math.abs(pnlPct) >= 1.0;
    if (!isNotable) return;

    const severity = args.pnl < 0 ? "loss" : (args.pnl > 0 ? "win" : "info");
    const lessonPrompt = `Сделка закрыта. Напиши ОДНО предложение (макс 180 символов, по-русски) — урок на будущее, который вытекает из этой сделки. Без воды, без преамбулы, в формате "Урок: ...".

Бумага: ${args.ticker}
Действие: ${args.action.toUpperCase()} ${args.qty} шт. @ ${args.entry.toFixed(2)}₽ → закрыто @ ${args.exit.toFixed(2)}₽
Результат: ${args.pnl >= 0 ? "+" : ""}${args.pnl.toFixed(2)}₽ (${pnlPct >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%) по причине ${args.closeReason}
Обоснование при входе: ${(args.reasoningAtEntry ?? "").slice(0, 400)}`;

    const resp = await withRetry<{ content: Array<{ type: string; text?: string }> }>(() => anthropic.messages.create({
      model: "claude-haiku-4-5",
      max_tokens: 200,
      messages: [{ role: "user", content: lessonPrompt }],
    }));
    const lesson = resp.content
    ...
    await db.insert(lessonsTable).values({
      ...
      relatedTradeLogId: args.relatedTradeLogId,
    });
    logger.info({ ticker: args.ticker, severity, pnl: args.pnl, lesson }, "Trade lesson recorded");
  } catch (err) {
    logger.error({ err, ticker: args.ticker }, "journalClosedTrade failed");
  }
}

/** Pull last N relevant lessons (ticker-specific + general), formatted for the prompt. */
async function getLessonsForPrompt(ticker: string, limit = 15): Promise<string> {
  const tickerLessons = await db.select().from(lessonsTable)
    .where(eq(lessonsTable.ticker, ticker))
    .orderBy(desc(lessonsTable.createdAt))
    .limit(8);
  const generalLessons = await db.select().from(lessonsTable)
    .where(isNull(lessonsTable.ticker))
    .orderBy(desc(lessonsTable.createdAt))
    .limit(7);
  const all = [...tickerLessons, ...generalLessons]
    .sort((a, b) => (b.createdAt > a.createdAt ? 1 : -1))
    .slice(0, limit);
  if (all.length === 0) return "Журнал уроков пуст — это первые сделки.";
  return all.map(l => {
    const dt = new Date(l.createdAt).toISOString().slice(0, 10);
    const tag = l.ticker ?? "общий";
    const sevMark = l.severity === "loss" ? "🔻" : l.severity === "win" ? "🟢" : "•";
    return `  ${sevMark} [${dt} ${tag}] ${l.lesson}`;
  }).join("\n");
}
...
async function analyzeOneTicker(target: { figi: string; ticker: string }, s: Awaited<ReturnType<typeof getOrCreateSettingsForLoop>>, ctx: AccountCtx, indexSnap: IndicatorSnapshot | null): Promise<CycleResult> {
  ...
  // STATIC system block — same string for every ticker in this cycle and every
  // cycle until risk parameters change. Marked cache_control so Anthropic
  // serves it from prompt cache (10x cheaper, ~250ms faster) for the next 4
  // minutes. Keep this block large and stable: identity, philosophy, schema.
  const staticSystem = `Ты — профессиональный алготрейдер на Московской Бирже (MOEX) с глубокой экспертизой технического и фундаментального анализа российского фондового рынка.

ТВОЯ РОЛЬ
— Принимай взвешенные торговые решения на основе данных, а не интуиции.
— Думай как риск-менеджер: сохранение капитала важнее доходности.
...
— Стоп-лосс по умолчанию: ${stopPctSuggest}% от цены входа (buy — ниже, sell — выше). Тейк-профит: ${takePctSuggest}%. Можешь сдвинуть, если уровни поддержки/сопротивления подсказывают иначе.
...
Поле reasoning — краткое (2-4 предложения) обоснование на русском с конкретными числами из данных.`;

  // DYNAMIC system block — context that changes per-ticker (positions, cash).
  // Not cached.
  const dynamicSystem = `КОНТЕКСТ СЧЁТА ДЛЯ ЭТОГО РЕШЕНИЯ
...
═══ ОБРАТНАЯ СВЯЗЬ ПО ТВОИМ ПРОШЛЫМ РЕШЕНИЯМ (КАЛИБРОВКА) ═══
${calibrationBlock}

═══ ЖУРНАЛ УРОКОВ (последние выводы из закрытых сделок) ═══
${lessonsBlock}
